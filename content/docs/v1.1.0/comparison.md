...
| **Raw/streamed responses**   | ✓ `.download(handler)`/`.upload(handler)` return a raw `Response`/`BodyInit`, bypassing JSON serialization entirely | Via `httpBatchStreamLink` for progressive batched query streaming                                                 |

**In short:** doofpi prioritizes **zero ceremony** and **runtime portability** over advanced
features like request batching and deep framework integrations.
...
- You need **progressive/batched streaming** of multiple queries over a single request via
  `httpBatchStreamLink` (tRPC v11) - doofpi's `.download()`/`.upload()` return one raw response per
  endpoint, they don't batch or progressively stream multiple procedure results
