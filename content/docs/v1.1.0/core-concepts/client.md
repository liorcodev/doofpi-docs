...
`.download()` always resolves to the raw `Response` object - it's never JSON-parsed, since the
server may return files or arbitrary bytes:
...
`.upload(input, body, requestInit?)` sends `body` as the raw request payload (the server's
handler reads it untouched via `req`), and always resolves to the raw `Response` object:
