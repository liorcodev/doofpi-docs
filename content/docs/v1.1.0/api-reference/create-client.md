...
### `.download(input?, requestInit?)`

Sends a `GET` request like `.read()`, but always resolves to the raw `Response` object - the body
is never JSON-parsed, since the server may return files or arbitrary bytes.
...
### `.upload(input, body, requestInit?)`

Sends a `POST` request with `body` as the raw request payload - the server's handler reads it
untouched via `req`. Like `.download()`, it always resolves to the raw `Response` object, never
JSON-parsed. `input` (if the endpoint has a `model.input` schema) is still serialized as
`?input=<JSON>` in the query string, just like `.read()`/`.download()`.

```ts
const res = await client.file.receipt.upload({ filename: 'photo.png' }, new Uint8Array([1, 2, 3]));
const result = await res.text();

// With RequestInit options
await client.file.receipt.upload(
  { filename: 'photo.png' },
